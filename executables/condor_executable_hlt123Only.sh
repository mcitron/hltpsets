...
function edit_pset {
    echo "modifyHLTforEras(process)" >> pset.py
    echo "_customInfo['maxEvents' ] = ${NEVTS}" >> pset.py
    echo "process.hltOutputFull.fileName = cms.untracked.string(\"${OUTPUTNAME}.root\")" >> pset.py

    if [[ "$INPUTFILENAMES" != "dummy"* ]]; then
        echo "_customInfo['inputFile' ] = cms.untracked.vstring([" >> pset.py
    ...
    echo "process = customizeHLTforAll(process,\"GRun\",_customInfo)" >> pset.py
    echo "process = customizeHLTforCMSSW(process,\"GRun\")" >> pset.py
}
function stageout {
    COPY_SRC=$1
    COPY_DEST=$2
