...
function edit_psets {
    ...
    if [ "$INPUTFILENAMES" != "dummyfile" ]; then 
        echo "process.source.fileNames = cms.untracked.vstring([" >> $hltcfg
    ...
    echo "process.TFileService.fileName = \"file:${OUTPUTNAME}.root\"" >> $triggerplotscfg

}

function setup_hlttriggerplots {
    pushd .
...
hltcfg="psets/datahlt_noThresh_cfg.py"
