...
function edit_psets {
    ...
    # hltstep
    # echo "_customInfo['maxEvents' ] = $nevents" >> $hltcfg
    # echo "_customInfo['inputFile' ]=  [\"file:output_recosim.root\"]" >> $hltcfg
    # echo "process.hltOutputMinimal.fileName = cms.untracked.string(\"file:output_hlt.root\")" >> $hltcfg
    ...
    echo "modifyHLTforEras(process)" >> $hltcfg
    echo "_customInfo['maxEvents' ] = $nevents" >> $hltcfg
    echo "process.hltOutputFull.fileName = cms.untracked.string(\"output_hlt.root\")" >> $hltcfg

    if [[ "$INPUTFILENAMES" != "dummy"* ]]; then
        echo "_customInfo['inputFile' ] = cms.untracked.vstring([" >> $hltcfg
        for INPUTFILENAME in $(echo "$INPUTFILENAMES" | sed -n 1'p' | tr ',' '\n'); do
            INPUTFILENAME=$(echo $INPUTFILENAME | sed 's|^/ceph/cms||')
            # INPUTFILENAME="root://xrootd.unl.edu/${INPUTFILENAME}"
            echo "\"${INPUTFILENAME}\"," >> $hltcfg
        done
        echo "])" >> $hltcfg
    fi
    echo "process = customizeHLTforAll(process,\"GRun\",_customInfo)" >> $hltcfg
    ...
    # skim
    echo "process.maxEvents.input = $nevents" >> $skimcfg
    echo "process.source.fileNames = [\"file:output_hlt.root\"]" >> $skimcfg
    echo "process.hltOutputMinimal.fileName = cms.untracked.string(\"file:${OUTPUTNAME}.root\")" >> $skimcfg


}

function setup_hltskim {
    pushd .
...
hltcfg="psets/hltDataFull_addLowThresh124X.py"
skimcfg="psets/skim_cfg.py"
